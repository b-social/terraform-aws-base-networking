locals {
  ...
  # subnet, route table, association and NAT gateway, is unchanged.
  availability_zone_limit = (
    var.availability_zone_limit == null ? 3 : var.availability_zone_limit
  )

  effective_availability_zones = slice(
    var.availability_zones,
    0,
    min(length(var.availability_zones), local.availability_zone_limit)
  )
}
